...
namespace Ast {
	...
	VariableValue::VariableValue(const std::vector<std::string>& dotted_ids) : dotted_ids(dotted_ids)
	{
	...
	ObjectHolder VariableValue::Execute(Closure& closure)
	{
		...
		for (auto it = dotted_ids.begin(); it != dotted_ids.end() - 1; it++)
		{
			if ((*current).count(*it))
			{
				...
				throw std::runtime_error("VariableError");
			}
		}
	...
	ObjectHolder FieldAssignment::Execute(Runtime::Closure& closure)
	{
		...
		for (const auto& item : object.dotted_ids)
		{
